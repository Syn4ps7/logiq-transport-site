  ...
  return <section id="tarifs" className="py-24 relative">
      <div className="container mx-auto px-6">
        ...
        <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
          ...
          duration: 0.5,
          delay: index * 0.1
        }} className={`relative rounded-lg p-6 ${plan.highlighted ? "bg-gradient-to-br from-primary/10 to-card border-2 border-primary glow-accent" : "surface-card border border-border"}`}>
              {plan.highlighted && <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 bg-primary text-primary-foreground text-xs font-semibold rounded-full">
                  Populaire
                </div>}
              ...
              <ul className="space-y-3 mb-8">
                {plan.features.map(feature => <li key={feature} className="flex items-center gap-3 text-sm">
        ...
        duration: 0.5,
        delay: 0.4
      }} className="text-center text-sm text-muted-foreground mt-8">km supplémentaire : 0.70 CHF. Correspond au coût réel d'usure et d'entretien du véhicule.</motion.p>
      </div>
    </section>;
};
